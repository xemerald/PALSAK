...
#define FD_SET(n, p)    ((p)->fds_bits[(n)/CHAR_BIT_CNT] |= (1 << ((n) % CHAR_BIT_CNT)))
#define FD_CLR(n, p)    ((p)->fds_bits[(n)/CHAR_BIT_CNT] &= ~(1 << ((n) % CHAR_BIT_CNT)))
#define FD_ISSET(n, p)  ((p)->fds_bits[(n)/CHAR_BIT_CNT] & (1 << ((n) % CHAR_BIT_CNT)))
...
/*
  type can use : SOCK_STREAM and SOCK_DGRAM. (definde below)
	protocol can use : TCP, UDP and ICMP. (definde below)
*/
/* socket types */
...
#define ARP 2                   /* Address Resolution Protocol */
#define TCP 3                   /* Transport Control Protocol: full connections */
#define UDP 4                   /* User Datagram Protocol: simple connections */
...
extern long MINTXTOUT;  /* minimum transmission timeout. default is 1000 ms(1 second).*/
extern long MAXTXTOUT;  /* maximum transmission timeout. default is 60000 ms (60 seconds).*/
extern int  RETX_THR_R2; /*re-transmission times, default is 12. 
                          1st timeout = MINTXTOUT
                          2nd timeout = 1st timeout *2
                          3rd timeout = 2nd timeout *2  */ 
...
extern long DNS_REPLY_TIMEOUT_ms; //[2008/11/13] add, ddefault is 0
extern unsigned DNS_REPLY_TIMEOUT; //[2008/11/13] add,default is 2 
extern unsigned DNS_MAX_RETRIES; 	//[2008/11/13] add, defalut is 5
...
/*
  [2004/12/10] add
  If the program will use the DHCP client function, call the function "Install_DHCP()".
  By default if the program without call "DHCPget()", the code for DHCP will not be 
...
/*
  on success return 0, and the filter value is assigned to the buffer pointed by ipf.
  on fail return -1.
*/
...
/* [2008/01/17] add variable for TCP KEEP ALIVE */
extern unsigned long TcpKeepAliveTime; /* default value is 60000,(for 60 seconds)*/
#define SOCKET_SET_TCP_KEEP_ALIVE_ON(s) connblo[s].txstat |= S_KEEPA
#define SOCKET_SET_TCP_KEEP_ALIVE_OFF(s) connblo[s].txstat &= ~S_KEEPA
...
/*[2009/03/10] add variable for ARP timeout */
extern unsigned long ARP_timeout;
/*
default value is 60000 (60000 ms=60 secs)
set ARP_timeout to 0 will disable the ARP timeout.
*/

/*[2009/05/27] add variable for the default TCP client tx timeout */
extern long INIT_TXVAR; /* default=3000UL; */

/*
[2010/06/07] add functions for mac address filter 
By default the mac address filter is disabled.
If the program only accept the packets come from some mac address, just use
the AddMacFilter() to add the mac address into the filter table.
After call the AddMacFilter(), only the source mac address is in the filter table
will be accepted, others will be filtered.

The maximum mac address filter number is 8.    
*/
int AddMacFilter(unsigned char *mac);
/*
 return value:
 0: Add mac filter success.
 1: the mac address already in the filter table.
-1: the filter table is full.
*/

int DeleteMacFilter(unsigned char *mac);
/*
 return value:
 0: delete mac filter success.
-1: the mac address did not in the filter table.
*/

extern int bAutoLink;  /*[2012/05/21] */
/*
1. Default value is 1.(enable)
2. When set bAutoLink=1 (for DM9000), and DM9000 go into un-link status,
   the driver will try to reset the PHY. Sometimes it can recover the un-link status.
*/

extern long MacWdtTimeout; /* [2012/05/30] */
/*
1. default value is 0L. (disable)
2. When set MacWdtTimeout > 0, if the mac did not receive any packet for the time
   MacWdtTimeout(milli seconds), the system will be reset by WDT.  
*/

extern long tAutoArpToGateway; /*[2012/05/30] add*/
/*
1.default value is 0L
2.when set tAutoArpToGateway > 0, every tAutoArpToGateway milliseconds will send
 an ARP packet out.(to get the mac address of the GATEWAY)
*/

/*[2012/06/13]*/
extern long MacRxTimeout; //=0; // 0:disable, >0:enable
extern int bWaitLinkOK;

extern char DHCP_HostName[31];
/*
User can define the host name used in the DHCP option 12.
Default is not used.
*/

extern int bDhcpUseDefaultHostName; /*[2013/09/24]*/
/*
Default value is 1, use the default host name "icpdas_xx:xx:xx"
Set bDhcpUseDefaultHostName to 0 and DHCP_HostName[0]=0 will disable the DHCP option 12.
*/

extern unsigned char IP_TTL; /* [2014/04/22] */
/*
Default value is 64.
*/ 
#ifdef __cplusplus
